...
const HistoryScreen = () => {
  ...
  }, [metric, preferences, route.params.unit]);
  ...
  const { chartData, rangeLabel, totalSum, average, trend, barConfig } = useMemo(() => {
    ...
    history.forEach(item => {
      ...
      // FIXED: Looks for BOTH value and count to support old and new database structures
      if (metric === 'steps' && item.type === 'steps') val = Number(item.value) || Number(item.count) || 0;
      
      if (metric === 'hydration' && (item.type === 'water' || item.type === 'hydration')) {
         val = Number(item.amount) || Number(item.value) || 0;
      }
    ...
    if (metric === 'steps' || metric === 'hydration') {
      ...
      if (currentVal > 0) {
        if (filter === 'D') {
           ...
           // Distribute current steps across the active hours of the day for a realistic 24hr chart
           if (metric === 'steps') {
             const startHour = 7; // Assume day starts at 7 AM
             const hoursActive = Math.max(currentHour - startHour + 1, 1);
             const avgPerHour = currentVal / hoursActive;
             for (let i = startHour; i <= currentHour; i++) {
                 if(buckets[i]) buckets[i].value = Math.max(buckets[i].value, avgPerHour);
             }
           }
        } else if (filter === 'W' || filter === 'M') {
    ...
    const firstHalfAvg = buckets.slice(0, mid).reduce((a,b)=>a+b.value,0) / Math.max(mid, 1);
    const secondHalfAvg = buckets.slice(mid).reduce((a,b)=>a+b.value,0) / Math.max(buckets.length - mid, 1);
