...
import { useRouter } from 'expo-router'; // <-- FIXED: Switched to Expo Router
...
const SummaryScreen = () => {
  ...
  const WeeklyChart = ({ history }) => {
    ...
    return (
      <TouchableOpacity 
        activeOpacity={0.9}
        // <-- FIXED: Updated navigation for history chart
        onPress={() => router.push({ 
          pathname: '/history', 
          params: { metric: 'calories', label: 'Active Energy', color: '#FF9500', unit: 'kcal' } 
        })}
      >
    ...
    return (
      <View style={[styles.rowContainer, isGap && styles.gapRowContainer]}>
        ...
        <View style={styles.rowContent}>
           <View style={[styles.dot, { backgroundColor: isGap ? (item.color || '#FF453A') : (item.color || colors.primary) }]} />
           <Text style={[styles.rowTitle, isGap && [styles.gapTitleText, { color: item.color || '#FF453A' }]]}>{item.title || "Free Time"}</Text>
        </View>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.largeCard}>
          ...
          {currentGap ? (
            <TouchableOpacity 
              style={[styles.perfectWindowBtn, { backgroundColor: currentGap.color || '#BB2D2D' }]} 
              activeOpacity={0.8} 
              onPress={() => router.push('/workout')} // <-- FIXED: Updated to router.push
            >
              <View style={styles.btnIconCircle}>
                <Ionicons name={currentGap.gapQuality === 'Diamond' ? 'diamond' : 'flash'} size={24} color="#FFF" />
              </View>
              <View style={{flex: 1}}>
                <Text style={styles.btnTitle}>
                  {currentGap.gapQuality ? `${currentGap.gapQuality} Window` : "You're Free Right Now!"}
                </Text>
                <Text style={styles.btnSub}>
                  {currentGap.duration} available • {currentGap.suggestion || 'Try a Micro-Workout'}
                </Text>
              </View>
        ...
        <View style={styles.midRow}>
          <TouchableOpacity 
            ...
            activeOpacity={0.7}
            // <-- FIXED: Updated to router.push
            onPress={() => router.push({ 
              pathname: '/history', 
              params: { metric: 'steps', label: 'Steps', color: colors.danger, unit: 'steps' } 
            })}
          >
...
const getStyles = (theme, colors) => StyleSheet.create({
  ...
  gapRowContainer: { backgroundColor: 'rgba(255, 59, 48, 0.05)', borderWidth: 1, borderColor: 'rgba(255, 59, 48, 0.1)' },
  ...
  perfectWindowBtn: { flexDirection: 'row', alignItems: 'center', padding: 20, borderRadius: 24, marginTop: 10, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 5 },
