        ...
        <View style={{ flex: 1 }}>
          {isGap ? (
            <TouchableOpacity 
              ...
              onPress={() => router.push('/workout')} // <-- FIXED: Updated to Expo Router push
              activeOpacity={0.8}
            >
               <View style={styles.gapHeaderRow}>
                  <Text style={styles.gapTitleText}>{item.title || "Free Time"}</Text>
                  <Text style={styles.gapDurationText}>{item.duration}</Text>
               </View>
               <View style={[styles.gapBadge, { backgroundColor: item.color || '#BB2D2D' }]}>
                  <Ionicons name={item.gapQuality === 'Diamond' ? 'diamond' : 'flash'} size={14} color="#FFF" style={{marginRight: 6}} />
                  <View>
                     <Text style={styles.gapBadgeTitle}>{item.gapQuality ? `${item.gapQuality} Window` : 'Perfect Window!'}</Text>
                     <Text style={styles.gapBadgeSub}>{item.suggestion || 'Try a Micro-Workout'}</Text>
                  </View>
               </View>
            </TouchableOpacity>
          ) : (
...
const getStyles = (theme, colors) => StyleSheet.create({
  ...
  gapBadge: { flexDirection: 'row', alignItems: 'center', borderRadius: 12, paddingVertical: 8, paddingHorizontal: 10 },
